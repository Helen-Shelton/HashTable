...
public class HashTableLin {
    ...
    public void insert(int n)
    {
        ...
        int index = n%sizeT;//calculates index key (n) will start at in table (hash function)
    ...
    private void rehash()
    {
        ...
        HashTableLin Table1 = new HashTableLin(maxKeys,maxLoad);//make new HashTable object that is at least twice the size of original table
        for(int i = 0; i<sizeT;i++)//for every element in the original hash table
        {
            if(Table[i] != -1)//if a nonempty space is found
                Table1.insert(Table[i]);//insert it into the new hash table. Insert function calclates new indices based on new hash function
        }
        Table = Table1.getTable();
        sizeT = Table1.getSizeT();
    }
    
    public boolean isIn(int n)
    {
        int i = n%sizeT, p = 1;//first index to search, first probe 
        while ((Table[i] != -1) && (p<sizeT))//while we have not probbed through the whole table or come across an empty index
        {
            ...
            {
                i = ++i % sizeT;//move to next index in table
